...
def consultar_saldo_eth(address):
    try:
        url = f"https://api.ethplorer.io/getAddressInfo/{address}?apiKey=freekey"
        r = requests.get(url, timeout=10).json()
        saldo = float(r.get('ETH', {}).get('balance', 0))
        return saldo
    except:
        return None
...
def mostrar_resultados(frase):
    ...
    salida_saldo.insert(tk.END, "\n🌐 Saldos en Ethereum y BNB:\n")
    for coin in ["Ethereum", "BNB"]:
        saldo = consultar_saldo_eth(direcciones[coin])
        if saldo is not None:
            salida_saldo.insert(tk.END, f" - {coin}: {saldo:.8f} ETH\n")
            if saldo > 0 and auto_save_saldo.get():
                guardar_frase(frase)
        else:
            salida_saldo.insert(tk.END, f" - {coin}: ❌ Error al consultar\n")
...
def generar():
    ...
    threading.Thread(target=mostrar_resultados, args=(frase,), daemon=True).start()
